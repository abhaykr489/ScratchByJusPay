...
function PreviewArea({ character, add_character, set_active }) {
  ...
  // showing message that new Script is added
  const addCharacterWithMessage = () => {
    add_character();
    setShowMessage(true);
    setTimeout(() => {
      ...
      <div className="flex justify-between mb-10">
        ...
        <div>
          <Button
            ...
            onClick={() => addCharacterWithMessage()}
          >
            Sprite{" "}
          </Button>
          {showMessage && (
            <div className="text-green-500 mt-2">New Sprite added</div>
          )}
        </div>
      </div>
